...
public class PolyRob {
	...
	private void simulationStep(int duration) {
		int startTime = vrep.simxGetLastCmdTime(clientID);
		int time = 0;
		do {
	...
	public void display(String instructionName) {
		String name = instructionName;
		if(name == "\"\"") name = "No label";
		String s = "Current instruction : "+name+"\n"
				+"Current position : ("+getPosition().x+", "+getPosition().y+")\n"
				+"Current orientation : "+Math.round(getOrientation()*180/Math.PI)+" degrees";
		vrep.simxAuxiliaryConsolePrint(clientID, consoleDebugHandle.getValue(), s+"\n", remoteApi.simx_opmode_blocking);
	}
	
	public void clearDisplay() {
		vrep.simxAuxiliaryConsolePrint(clientID, consoleDebugHandle.getValue(), null, remoteApi.simx_opmode_blocking);
	}
